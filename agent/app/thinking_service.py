...
async def abstract_direction_from_dialog(type, robot, loop):
    async for uid, history in scan_dialog(type, loop):
        ...
        if history and len(history) > 0:
            ...
            if len(unprocessed_history) > 0:
                request = ChatRequest(scene="tools", robot="profile_build", mode="complete", content=json.dumps(unprocessed_history), user="")
...
def job_build_profile():
    ...
    loop.run_until_complete(abstract_data_from_dialog_v2("mysql", "humi", loop))
